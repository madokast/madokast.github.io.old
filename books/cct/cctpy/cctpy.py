...
class ParticleRunner:
    ...
    """
    @staticmethod
    def __callback_for_runge_kutta4(particle: RunningParticle, magnet: Magnet) -> Callable[[float, numpy.ndarray], numpy.ndarray]:
        """
    ...
    @staticmethod
    def __callback_for_solve_ode(particle: RunningParticle, magnet: Magnet) -> Callable[[float, numpy.ndarray], numpy.ndarray]:
        """
    ...
    def run_only_ode(
        ...
        if isinstance(p, RunningParticle):
            dt = footstep/p.speed
            t_end = length/p.speed
            Y0 = numpy.array([p.position.x,p.position.y,p.position.z, p.velocity.x,p.velocity.y,p.velocity.z])
            func = ParticleRunner.__callback_for_solve_ode(
                particle=p, magnet=m)
            Y1 = BaseUtils.solve_ode(
                t0=0.0, t_end=t_end, Y0=Y0, y_derived_function=func, dt=dt, record=False, absolute_tolerance=absolute_tolerance, relative_tolerance=relative_tolerance)
...
class Beamline(Line2, Magnet, ApertureObject):
    ...
    def track_phase_ellipse(
        ...
        ParticleRunner.run_only(
            p=rp_x, m=self, length=length, footstep=footstep)

        ParticleRunner.run_only(
            p=rp_y, m=self, length=length, footstep=footstep)
...
class BaseUtils:
    ...
    @staticmethod
    def runge_kutta4(t0: float, t_end: float, Y0: T, y_derived_function: Callable[[float, T], T], dt: float, record: bool = False) -> Union[T, Tuple[List[float], List[T]]]:
    ...
    @staticmethod
    def solve_ode(t0: float, t_end: float, Y0: T, y_derived_function: Callable[[float, T], T], dt: float, record: bool = False, absolute_tolerance: float = 1e-8, relative_tolerance: float = 1e-8) -> Union[T, Tuple[List[float], List[T]]]:
        if record:
            raise NotImplementedError
            # number: int = math.ceil((t_end-t0)/dt)
